describe('Discount management view', function () {
	...
	it("shouldn't load the discounts due to not authenticated used", function (){
		browser.get('http://localhost:3000/management/discounts');

		expect(browser.getCurrentUrl()).toBe('http://localhost:3000/401');
	});

	it("shouldn't load the discounts to a customer", function (){
	...
	it("shouldn't load the discounts to a supplier", function (){
		browser.get('http://localhost:3000/signin');
