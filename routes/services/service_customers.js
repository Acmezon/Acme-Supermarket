...
exports.checkFieldsCorrect = function(name, surname, email, password, coordinates, address, country, city, phone) {
	...
	return validator.isEmail(email) && password && password.length>=8 && password.length<=32 && /^(\-?\d+(\.\d+)?);(\-?\d+(\.\d+)?)$/.test(coordinates)
			&& countries.indexOf(country) > -1 && phone && phone.length >= 9 && phone.length <= 15;
}